...
def main():
    ...
    output_filename = "resnet_output.txt"
    
    # 1. Load Data
    X_train_np, y_train_np, X_val_np, y_val_np = utils.process_data_augmented()
    ...
    # 2. Init Model
    model = GomokuResNet().to(config.DEVICE)
    ...
    pbar = tqdm(range(config.EPOCHS_AUG), unit="epoch")
    ...
    # 4. Save model
    torch.save(model.state_dict(), config.RESNET_PATH)
    print(f"Model saved as '{config.RESNET_PATH}'")
