...
import config
from model import GomokuMLP
import utils

def main():
    print(f"Device: {config.DEVICE}")
    
    output_filename = "mlp_output.txt"
    ...
    # 2. Init Model
    model = GomokuMLP().to(config.DEVICE)
    ...
    # 4. Save model
    torch.save(model.state_dict(), config.MLP_PATH)
    print(f"Model saved as '{config.MLP_PATH}'")
